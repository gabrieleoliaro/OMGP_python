...
function  [loghyper, convergence] = omgpE(loghyper, covfunc, M, X, Y)
...
for iter=1:maxit
    ...
    for m = 1:M % Consider updating a single function at a time (K-means style).
        if length(covfunc) > 1
    ...
    logqZ = a +logpZ - oD/2*log(2*pi*sn2);
    logqZ = logqZ - max(logqZ,[],2)*ones(1,M);logqZ = logqZ-log(sum(exp(logqZ),2))*ones(1,M);qZ=exp(logqZ);
    
    if abs(oldF-oldFant)<abs(oldFant)*1e-10
        break
    end
    oldFant = oldF;
end
...
logqZ = logqZ(:,2:end);
logqZ(logqZ>100) = 100;logqZ(logqZ<-100) = -100;
